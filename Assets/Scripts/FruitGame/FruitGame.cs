...
public class FruitGame : MonoBehaviour
{
    ...
    void SpawnnewFruit()
    {
        if (!isGameOver)     //게임 오버가 아닐 때만 새 과일 생성
        {
            ...
            Vector3 spawnPosition = new Vector3(worldPosition.x, fruitStartHeight, 0);    //X 좌표만 사용 하고 나머지는 설정한 값으로 진행 한다.
    ...
    private void Update()
    {
        ...
        {
            Vector3 mousePosition = Input.mousePosition;    //마우스 2D 위치를 월드 3D 좌표로 변환
            Vector3 worldPosition = mainCamera.ScreenToWorldPoint(mousePosition);       //마우스 2D 위치를 월드 3D 좌표로 변환
    ...
    void DropFruit()
    {
        ...
        if (rb != null)
        {
            rb.gravityScale = 1.0f;  //중력을 원래 값으로 복구 시킨다.
    ...
    {
        if(fruitType < fruitSize.Length - 1)    //마지막 과일 타입이 아니라면
        {
            GameObject newFruit = Instantiate(fruitperfabs[fruitType + 1], position, Quaternion.identity);      //해당 타입 나은 번호를 생성 시킨다.
